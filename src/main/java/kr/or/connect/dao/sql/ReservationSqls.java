...
public class ReservationSqls {
    ...
    public static String selectByCategoryId() {
        return new StringBuilder().append("SELECT P.ID, P.CATEGORY_ID, P.DESCRIPTION,P.CONTENT, P.EVENT")
                .append(", C.NAME , I.FILE_ID")
                .append(", D.ID AS DISPLAYINFOID , D.OPENING_HOURS, D.PLACE_NAME, D.PLACE_LOT, D.PLACE_STREET, D.TEL, D.HOMEPAGE, D.EMAIL, D.CREATE_DATE, D.MODIFY_DATE")
                .append(" FROM PRODUCT P").append(" INNER JOIN CATEGORY C").append(" ON C.ID = P.CATEGORY_ID ")
                .append(" INNER JOIN DISPLAY_INFO D").append(" ON P.ID = D.PRODUCT_ID")
                .append(" INNER JOIN PRODUCT_IMAGE I").append(" ON P.ID = I.PRODUCT_ID AND I.TYPE = 'MA'")
                .append(" WHERE").append(" P.CATEGORY_ID IN (:categoryId)").append(" ORDER BY")
                .append(" P.CATEGORY_ID =:categoryId").append(" DESC limit :start, :limit").toString();
    }
